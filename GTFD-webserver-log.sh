...
dis_cups_p631() {
    ...
    log "[WARNING] Disabling CUPS on port 631!" "$oO_LOG_FILE"
    echo "[WARNING] Disabling CUPS on port 631!"

    # CUPS stoppen
    if sudo systemctl stop cups; then
        log "CUPS service has been stopped successfully." "$oO_LOG_FILE"
    else 
    ...
    if sudo systemctl disable cups; then
        log "CUPS service has been disabled. Port 631 is now closed." "$oO_LOG_FILE"
    else    
...
nginx_conf() {
    ...
    if [ -L "$default_site" ] || [ -f "$default_site" ]; then
        ...
        rm "$default_site"
        log "$LOG_LEVEL_WARNING" "Default site removed successfully." "$oO_LOG_FILE"
    else
        log "$LOG_LEVEL_WARNING" "Default site not found at $default_site. No action needed." "$oO_LOG_FILE"
    fi
    ...
    if [ ! -f /etc/ssl/private/nginx-selfsigned.key ] || [ ! -f /etc/ssl/certs/nginx-selfsigned.crt ] || [ ! -f /etc/ssl/certs/dhparam.pem ]; then
        ...
        log "$LOG_LEVEL_WARNING""SSL certificates already exist. Skipping generation." "$oO_LOG_FILE"
    fi
    ...
    if ! grep -q "limit_req_zone" "$nginx_global_conf"; then
        ...
        log "$LOG_LEVEL_WARNING" "Rate limiting directive already exists in Nginx configuration, skipping." "$oO_LOG_FILE"
    fi
    ...
    if ! grep -q "log_format proxy_logs" "$nginx_global_conf"; then
        ...
        log "$LOG_LEVEL_WARNING" "Log_format directive already exists in Nginx configuration, skipping." "$oO_LOG_FILE"
    fi
...
EOF
    ...
    if [ ! -f "$NGX_SITES_ENABLED" ]; then
        ...
        log "$LOG_LEVEL_WARNING" "Nginx site configuration already enabled, skipping." "$oO_LOG_FILE"
    fi
    ...
    log "$LOG_LEVEL_WARNING" "Nginx SSL with strict HTTPS and SOCKS5 proxy configured successfully." "$oO_LOG_FILE"
    return 0
}
