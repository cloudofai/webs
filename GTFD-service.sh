...
installer() {
    ...
    local attempts=0
    local max_attempts=3
    local critical_packages=("curl" "tor" "nmap" "resolvconf" "nftables" "proxychains" "apt-transport-https" "jq" "git" "gzip" "iproute2")
...
verifier() {
    ...
    # Funktion zur Überprüfung der Tor-Funktionalität
    tor_check() {
        if ! command -v curl &> /dev/null; then
            echo "Error: curl is not installed. Please install curl to perform this check."
            return 1
        fi

        local output
        output=$(curl --socks5-hostname 127.0.0.1:9050 -s https://check.torproject.org)
        if echo "$output" | grep -q "Congratulations"; then
            echo "Congratulations! - This browser is configured to use Tor.."
            echo "By checking: check.torproject.org - Tor is working correctly."
            echo "Tor can't help you if you use it wrong! Learn how to be safe at https://support.torproject.org/faq"
        else
            echo "Error: Tor validation failed."
            return 1
        fi
    }
    ...
    # Funktion zur Überprüfung der DNS-Auflösung über Tor
    dns_check() {
        if ! command -v dig &> /dev/null; then
            echo "Error: dig is not installed. Please install dnsutils to perform this check."
            return 1
        fi

        local tmpfile
        tmpfile=$(mktemp /tmp/dns_check.XXXXXX)
        if dig @127.0.0.1 -p 5353 www.torproject.org > "$tmpfile"; then
            cat "$tmpfile"
            rm -f "$tmpfile"
        else
            echo "Error: DNS query failed. Check your Tor configuration."
            rm -f "$tmpfile"
            return 1
        fi
    }
    ...
    validate_ipv6() {
        ...
        if sysctl net.ipv6.conf.all.disable_ipv6 | grep -q "1" && \
            sysctl net.ipv6.conf.default.disable_ipv6 | grep -q "1" && \
            sysctl net.ipv6.conf.lo.disable_ipv6 | grep -q "1"; then
            echo "IPv6 successfully disabled and verified."
            return 0
        else
            echo "IPv6 might still be active. Verification failed.!"
            return 1
        fi
    }
...
# Laufzeit in ein lesbares Format umwandeln (optional)
...
log $LOG_LEVEL_INFO "created by ka5oeze" "$oO_LOG_FILE"
